...
def next_turn(snake, food):
    ...
    if x == food.coordinates[0] and y == food.coordinates[1]:
        global score
        score += 1
        label.config(text=f"Score: {score}", font=("Courier", 30))
        canvas.delete("food")
        food = Food()
    else:
...
def game_over():
    ...
    canvas.create_text(canvas.winfo_width() // 2, canvas.winfo_height() // 2,
                       fill="#DF1A2F", text="Game Over!", font=("Terminal", 60), tags="gameover")
...
GAME_WITH = 600
GAME_HEIGHT = 600
SPACE_SIZE = 25  #BAKHSH PAZIR BASHEH BE 2 TA BALA
SLOWNESS = 200
SNAKE_COLOR = "yellow"
BODY_SIZE = 2
FOOD_COLOR = "red"
